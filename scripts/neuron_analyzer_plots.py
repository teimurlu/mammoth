...
base_path = Path(
    "/Users/ibrahimteymurlu/Documents/University of Eindhoven/Master Thesis/Repos/mammoth/task_accuracies/seeds/49_20250318_0630_resnet18/analysis/"
)
accuracy_file = base_path / "accuracies.csv"
inactive_file = base_path / "inactive_neurons.csv"
# reviving_file = base_path / "reviving_neurons.csv"
...
# Add component columns to dataframes
# for df in [accuracies, inactive_neurons, reviving_neurons]:
#     if "model" in df.columns:
#         df["activation"] = df["model"].apply(
#             lambda x: parse_model_name(x)["activation"]
#         )
#         df["threshold"] = df["model"].apply(lambda x: parse_model_name(x)["threshold"])
#         df["strategy"] = df["model"].apply(lambda x: parse_model_name(x)["strategy"])
#         df["optimizer"] = df["model"].apply(lambda x: parse_model_name(x)["optimizer"])
...
def plot_accuracy_trends():
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "task_il_progression.png", dpi=300, bbox_inches="tight")
    ...
    plt.legend(bbox_to_anchor=(1.05, 1), loc="upper left", fontsize="small")
    plt.tight_layout()
    plt.savefig(output_dir / "class_il_progression.png", dpi=300, bbox_inches="tight")
...
def analyze_inactive_neurons():
    ...
    if "activation_x" in merged_data.columns and "activation_y" in merged_data.columns:
    ...
    # Same for optimizer
    if "optimizer_x" in merged_data.columns and "optimizer_y" in merged_data.columns:
    ...
    class_il_accuracy_task5 = avg_class_il_acc[avg_class_il_acc["num_tasks_seen"] == 5]
    ...
    if (
        ...
        merged_class_il_data.drop(["optimizer_x", "optimizer_y"], axis=1, inplace=True)
...
class_il_table = create_class_il_table()  # Add this line
plot_accuracy_trends()
inactive_progression = analyze_inactive_neurons()
# reviving_analysis = analyze_reviving_neurons()
...
# reviving_analysis.to_csv(output_dir / "reviving_neurons_by_task.csv")
# summary.to_csv(output_dir / "performance_summary.csv", index=False)

# Print summary information
print("Analysis complete!")
print(f"Output files saved to: {output_dir}")
print("\nTop 5 Methods by Final Task-IL Accuracy:")
# print(
#     summary[
#         ["model", "Final_Accuracy", "Inactive_Neurons_Final", "Reviving_Neurons_Task4"]
#     ]
#     .head(5)
