...
class ContinualModel(nn.Module):
    ...
    )  # (optional) The scheduler for the optimizer. If defined, it will overwrite the one defined in the `dataset`
    ...
    original_transform: (
        transforms.Compose
    )  # The original transformation to be applied to the input data. This is the one defined by the `dataset`
    ...
    def __init__(
        ...
        if dataset is None:
            logging.error(
                "No dataset provided. Will create another instance but NOTE that this **WILL** result in some bugs and possible memory leaks!"
            )
        ...
        try:
            ...
            self.normalization_transform = to_kornia_transform(
                self.dataset.get_normalization_transform()
            )
        except BaseException:
            logging.error("could not initialize kornia transforms.")
            self.normalization_transform = (
                transforms.Compose(
                    [transforms.ToPILImage(), self.dataset.TEST_TRANSFORM]
                )
        ...
        if self.net is not None:
            ...
            logging.warning(
                "no default model for this dataset. You will have to specify the optimizer yourself."
            )
            self.opt = None
        ...
        if self.args.label_perc != 1 and "cssl" not in self.COMPATIBILITY:
            logging.info(
                "label_perc is not explicitly supported by this model -> training may break"
            )
    ...
    def load_buffer(self, buffer):
        ...
        if isinstance(buffer, Buffer):
            assert (
                buffer.examples.shape[0] == self.args.buffer_size
            ), "Buffer size mismatch. Expected {} got {}".format(
    ...
    def get_optimizer(
        self, params: Iterator[torch.Tensor] = None, lr=None
    ) -> optim.Optimizer:
        """
        ...
        if self.args.optimizer.lower() in supported_optims:
            if self.args.optimizer.lower() == "sgd":
                opt = getattr(optim, supported_optims[self.args.optimizer.lower()])(
                    params,
                    lr=lr,
                ...
                self.args.optimizer.lower() == "adam"
                or self.args.optimizer.lower() == "adamw"
            ):
                opt = getattr(optim, supported_optims[self.args.optimizer.lower()])(
                    params, lr=lr, weight_decay=self.args.optim_wd
                )
            ...
            if (~labeled_mask).any():  # if there are any unlabeled samples
                ...
                args = [
                    (
                        ...
                        and arg.shape[0] == args[0].shape[0]
                        else arg
                    )
                    for arg in args
                ]
    ...
    def meta_begin_task(self, dataset):
        ...
        self._epoch_iteration = 0
        self._past_epoch = 0
        self._n_classes_current_task = (
            self._cpt if isinstance(self._cpt, int) else self._cpt[self._current_task]
        )
        self._n_past_classes, self._n_seen_classes = self.compute_offsets(
            self._current_task
        )
        ...
        if not hasattr(self, "scheduler") or self.scheduler is None:
            ...
            logging.warning(
                "Model defines a custom scheduler. The optimizer will not be reloaded."
            )
    ...
    def autolog_wandb(self, locals, extra=None):
        ...
        if not self.args.nowand and not self.args.debug_mode:
            tmp = {
                ...
                if k.startswith("_wandb_") or "loss" in k.lower()
            }
            tmp.update(extra or {})
            if hasattr(self, "opt"):
